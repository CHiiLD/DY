...
namespace DY.NET.LSIS.XGT
{
    ...
    {
        public IList<IProtocolItemWithType>[] PackProtocolData(IList<IProtocolItemWithType> newList)
        {
            ILookup<Type, IProtocolItemWithType> look = SeparateProtocol(newList);
            List<List<IProtocolItemWithType>> package = new List<List<IProtocolItemWithType>>();
            foreach (IGrouping<Type, IProtocolItemWithType> groups in look)
            {
                List<IProtocolItemWithType> pack = new List<IProtocolItemWithType>();
                package.Add(pack);
                int cnt = 0;
                foreach (IProtocolItemWithType group in groups)
                {
                    if (cnt % 16 == 0 && cnt != 0)
                    {
                        pack = new List<IProtocolItemWithType>();
                        package.Add(pack);
                    }
            ...
            List<IProtocolItemWithType>[] result = package.ToArray();
            return result;
        }

        public void ConvertGlopaVariableName(IList<IProtocolItemWithType> newList)
        {
        ...
        public ILookup<Type, IProtocolItemWithType> SeparateProtocol(IList<IProtocolItemWithType> list)
        {
            ...
            ILookup<Type, IProtocolItemWithType> look = list.ToLookup(x => x.Type, x => x);
            return look;
        }
